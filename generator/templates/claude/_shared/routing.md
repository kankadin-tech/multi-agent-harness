...
- **gemini**: 단일 브리지 `mcp__gemini-pro__*` 하나만 사용 (CLI 래퍼 `mcp__gemini__*`는 폐기 — 잔존 참조 시 호출 실패). 기본 모델 `gemini-3.1-pro-low`, 빠른 응답은 같은 브리지에서 `model: gemini-3-flash`. `gemini-3.1-pro-high`는 로컬 프록시에서 `400 INVALID_ARGUMENT`가 재현되어 **기본·폴백 경로에서 제외**(사용자 명시 요청 시만). 기본값 `pro-low`는 프록시 capability에 종속 — 평소엔 안정 고정이고 **프록시 업그레이드 시에만 재확인·갱신**한다(claude/codex처럼 자주 안 변함). 근거: `_shared/learnings.md` [2026-05-15] · `design-basis.md` D4.
